...
func _setup_enemy_info_menu() -> void:
	...
	if is_instance_valid(top_header):
	...
	# Main Dropdown Panel Container (Maintained as PanelContainer to match variable type)
	_info_menu_panel = PanelContainer.new()
	_info_menu_panel.name = "EnemyInfoDropdownPanel"
	_info_menu_panel.add_theme_stylebox_override("panel", StyleBoxEmpty.new())
	add_child(_info_menu_panel)

	# Enable pointing hand cursor and click listener on the dropdown panel to slide up when clicked
	_info_menu_panel.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	if not _info_menu_panel.gui_input.is_connected(_on_top_header_gui_input):
		_info_menu_panel.gui_input.connect(_on_top_header_gui_input)

	# Anchor directly to top-center of screen over topbar
	_info_menu_panel.layout_mode = 1
	_info_menu_panel.anchors_preset = Control.PRESET_CENTER_TOP
	_info_menu_panel.offset_left = -300.0
	_info_menu_panel.offset_top = 0.0
	_info_menu_panel.offset_right = 300.0
	_info_menu_panel.grow_horizontal = Control.GROW_DIRECTION_BOTH
	_info_menu_panel.z_index = 10

	# Outer VBox containing content panel + bottom accent image
	var outer_vbox := VBoxContainer.new()
	outer_vbox.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	outer_vbox.add_theme_constant_override("separation", 0)
	_info_menu_panel.add_child(outer_vbox)

	# Dark cyber content panel for enemy list
	var content_panel := PanelContainer.new()
	content_panel.name = "ContentPanel"
	content_panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL

	# Style dark cyber panel (side borders only, leaving bottom open for accent PNG)
	...
	# Bottom Accent Image (hud_infomenu_btm.png) - Placed below content_panel as the bottom border cap
...
func _rebuild_enemy_info_rows() -> void:
	...
	for child in _info_enemy_vbox.get_children():
		_info_enemy_vbox.remove_child(child)
		child.queue_free()

	_info_enemy_rows.clear()

	# Load monospaced font for HP numbers and status text
	var mono_font: Font = null
	if ResourceLoader.exists("res://ui/fonts/RedditMono-Regular.ttf"):
		mono_font = load("res://ui/fonts/RedditMono-Regular.ttf") as Font
	...
	for i in range(_current_enemy_resources.size()):
		...
		# Enemy Name (Maintains standard font)
		...
		# Styled Cyber Progress Bar (Green Fill & Centered)
		...
		bg_style.bg_color = Color(0.08, 0.12, 0.15, 0.9)
		bg_style.border_color = Color(0.15, 0.35, 0.4, 0.8)
		...
		fill_style.bg_color = Color(0.12, 0.88, 0.42, 1.0)
		fill_style.set_corner_radius_all(2)
		hp_bar.add_theme_stylebox_override("fill", fill_style)

		top_row.add_child(hp_bar)

		# HP Label (Monospaced Font)
		...
		if is_instance_valid(mono_font):
			hp_lbl.add_theme_font_override("font", mono_font)
		top_row.add_child(hp_lbl)

		# Status & Buff Row (Monospaced Font)
		...
		if is_instance_valid(mono_font):
			status_lbl.add_theme_font_override("font", mono_font)
