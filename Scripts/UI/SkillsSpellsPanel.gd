	...
	config_btn.pressed.connect(
		func() -> void:
			if get_tree().root.has_node("SignalBus"):
				...
				# Pass both the character and the current_mode in the payload
		...
		if skills.is_empty():
			...
			for item in skills:
				if item is SkillData:
					...
					_create_entry(item.skill_name, sub_info, item.icon if "icon" in item else null, item)
	else:
		...
		if spells.is_empty():
			...
			for item in spells:
				if item is SpellData:
					...
					_create_entry(item.spell_name, sub_info, item.icon if "icon" in item else null, item)
...
func _create_entry(title_text: String, desc_text: String, icon_tex: Texture2D, data_resource: Resource) -> void:
	...
	if entry_node is Button:
		entry_node.pressed.connect(
			func() -> void:
				if get_tree().root.has_node("SignalBus"):
					print("Character '", current_cat.name if is_instance_valid(current_cat) else "null", "' selected ability: ", title_text)
					var bus: Node = get_tree().root.get_node("SignalBus")
